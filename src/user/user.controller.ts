...
export class UserController {
  ...
  @Patch('tours/members/:id')
  async updatesTourMembers(@Param('id') id: string, @Body() user: UserRequest){
    user.userId = id;
    return await this.userService.updateUser(plainToInstance(UserRequest, user), id);
  }

  @ApiOperation({ summary: 'Updates a tour admin' })
  @Permissions('create:organization')
  @Patch('tours/admins/:id')
  async updatesTourAgent(@Param('id') id: string, @Body() user: UserRequest){
    return await this.userService.updateUser(plainToInstance(UserRequest, user), id);
  }
}
