...
export class Auth0Service{
  ...
  public async patchUser(user: UserRequest, id: string) {
    const endpoint = this.endpointProvider('users/:id', {pathParams: {id: id}});
    let body = instanceToPlain(user);
    return await lastValueFrom(this.httpService.patch(endpoint, body).pipe(
        map(res => res.data)
    ));
  }
}
